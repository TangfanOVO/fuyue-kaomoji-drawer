...
import type {
  KaomojiCatalogEntry,
  KaomojiCatalogManifest,
  KaomojiCatalogMergeResult,
  KaomojiCatalogOptions,
  KaomojiCatalogSyncMode,
  KaomojiCatalogSyncState,
  KaomojiRepository,
} from "./types.js";
...
export function readKaomojiCatalogSyncState(storageKey = defaultKaomojiCatalogStateStorageKey): KaomojiCatalogSyncState {
  const fallback: KaomojiCatalogSyncState = { mode: "manual" };
  try {
    const raw = storage()?.getItem(storageKey);
    if (!raw) return fallback;
    const parsed = JSON.parse(raw) as Partial<KaomojiCatalogSyncState>;
    const mode: KaomojiCatalogSyncMode = ["manual", "automatic", "off"].includes(String(parsed.mode))
      ? parsed.mode as KaomojiCatalogSyncMode
      : "manual";
    return {
      mode,
...
export function writeKaomojiCatalogSyncState(state: KaomojiCatalogSyncState, storageKey = defaultKaomojiCatalogStateStorageKey) {
  storage()?.setItem(storageKey, JSON.stringify(state));
}

function validManifest(value: unknown): value is KaomojiCatalogManifest {
  if (!value || typeof value !== "object") return false;
  const manifest = value as Partial<KaomojiCatalogManifest>;
...
function validEntries(value: unknown): KaomojiCatalogEntry[] {
  if (!Array.isArray(value) || value.length > 10_000) throw new Error("精选库数据格式不正确");
  const seen = new Set<string>();
  const entries: KaomojiCatalogEntry[] = [];
  for (const candidate of value) {
    if (!candidate || typeof candidate !== "object") continue;
    const raw = candidate as Partial<KaomojiCatalogEntry>;
    if (typeof raw.value !== "string" || !Array.isArray(raw.categories)) continue;
    const value = normalizeKaomoji(raw.value);
    const categories = normalizeKaomojiCategories(raw.categories.filter((part): part is string => typeof part === "string"));
    const analysis = analyzeKaomoji(value, categories);
    if (!value || value.length > 500 || !categories.length || analysis.compatibility === "blocked" || seen.has(value)) continue;
    seen.add(value);
...
async function fetchJson(fetcher: typeof fetch, url: string, maxCharacters: number) {
  const parsed = new URL(url);
  if (parsed.protocol !== "https:") throw new Error("精选库地址必须使用 HTTPS");
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 10_000);
  try {
    const response = await fetcher(parsed.href, { headers: { Accept: "application/json" }, signal: controller.signal });
    if (!response.ok) throw new Error(`精选库连接失败（${response.status}）`);
    const text = await response.text();
    if (text.length > maxCharacters) throw new Error("精选库数据过大，已停止同步");
...
async function mergeFallback(repository: KaomojiRepository, entries: KaomojiCatalogEntry[]): Promise<KaomojiCatalogMergeResult> {
...
export async function syncKaomojiCatalog(repository: KaomojiRepository, options: KaomojiCatalogOptions = {}) {
  const fetcher = options.fetcher ?? globalThis.fetch;
  if (!fetcher) throw new Error("当前环境不支持联网同步");
  ...
  if (!validManifest(manifestRaw)) throw new Error("精选库清单格式不正确");
  ...
  if (entries.length !== manifestRaw.itemCount) throw new Error("精选库条目数校验失败");
...
export function shouldAutomaticallySync(state: KaomojiCatalogSyncState, options: KaomojiCatalogOptions = {}, now = Date.now()) {
