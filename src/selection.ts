...
function recentPenalty(item: KaomojiItem, now: number) {
  const usedAt = item.lastUsedAt ? Date.parse(item.lastUsedAt) : Number.NaN;
  if (!Number.isFinite(usedAt)) return 1;
  const age = Math.max(0, now - usedAt);
  if (age < 2 * 60_000) return 0.08;
  if (age < 60 * 60_000) return 0.35;
  if (age < 24 * 60 * 60_000) return 0.7;
  return 1;
}

export function selectDiverseKaomoji(
  ranked: KaomojiItem[],
  recentValues: string[] = [],
  variety: KaomojiVariety = "balanced",
  ...
  if (pool.length > 1) {
    ...
    if (unseen.length >= 2 || unseen.length === pool.length) pool = unseen;
  }
